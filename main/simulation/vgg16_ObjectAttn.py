...
astrgs = np.arange(0, 1., 0.5)  # attention strengths (betas)
...
if __name__ == '__main__':
    ...
    tn_batch = np.zeros((bsize, 224, 224, 3))
    tnlabs = np.zeros((bsize, 1))
    TPscore = np.zeros((len(astrgs)))
    TNscore = np.zeros((len(astrgs)))  # FN=np.zeros((len(str_vec),xval)); TN=np.zeros((len(str_vec),xval))

    if rec_activity:
        tp_resps1 = np.ones((2, bsize, 64, len(astrgs)))
        tp_resps2 = np.ones((2, bsize, 128, len(astrgs)))
        tp_resps3 = np.ones((3, bsize, 256, len(astrgs)))
        tp_resps4 = np.ones((3, bsize, 512, len(astrgs)))
        tp_resps5 = np.ones((3, bsize, 512, len(astrgs)))
        tn_resps1 = np.ones((2, bsize, 64, len(astrgs)))
        tn_resps2 = np.ones((2, bsize, 128, len(astrgs)))
        tn_resps3 = np.ones((3, bsize, 256, len(astrgs)))
        tn_resps4 = np.ones((3, bsize, 512, len(astrgs)))
        tn_resps5 = np.ones((3, bsize, 512, len(astrgs)))
    ...
    with sess.as_default():
        ...
        othercs = np.arange(Ncats)
        othercs = othercs[othercs != descat]
        cii = 0
        if imtype == 3:
            othercs = np.random.choice(othercs, 15, replace=False)
        else:
            # imspercats is already an integer array
            pass  # No need to modify here

        print Ncats, type(descat), othercs
        for ci in range(len(othercs)):
            if imtype == 1:
                ocatpicsF = np.load(impath + '/merg5_c' + str(othercs[ci]) + '.npz')
                ocatpics = ocatpicsF['arr_0']
                ocatlabs = ocatpicsF['arr_1']
                noncind = np.where(ocatlabs[0, :] != descat)
                del ocatpicsF

                # Ensure imspercats[ci] is an integer
                tn_batch[cii:cii + imspercats[ci], :, :, :] = ocatpics[
                    np.random.choice(15, int(imspercats[ci]), replace=True), 
                    np.random.choice(noncind[0], int(imspercats[ci]), replace=True), 
                    :, :, :
                ]
                del ocatpics
                cii += imspercats[ci]
                ...
                for pici in range(int(imspercats[ci])):
                    pic = np.random.choice(15, replace=True)
                    binocat = np.sum(np.squeeze(ocatlabs[pic, :, :]) == descat, axis=1)
                    print binocat	
                    noncind = np.where(binocat == 0)  # need to check that all 3 others aren't descat
                    if len(noncind[0]) == 0:
                        print("No valid non-descat indices found for pic:", pic)
                        continue  # Skip if no valid indices
                    tn_batch[cii + pici:cii + pici + 1, :, :, :] = ocatpics[
                        pic, 
                        np.random.choice(noncind[0], replace=True), 
                        :, :, :
                    ]
                del ocatpics
                cii += imspercats[ci]
                ...
                tn_batch[ci, :, :, :] = ocatpicsF[np.random.choice(15, replace=True), :, :, :]
                del ocatpicsF
        ...
        for astrg in astrgs: 
            ...
            # Evaluate TP and TN scores
            tp_score = vgg.guess.eval(feed_dict={
                vgg.imgs: tp_batch, 
                vgg.a11: attnmats[0],
                vgg.a12: attnmats[1],
                vgg.a21: attnmats[2],
            ...
            tn_score = vgg.guess.eval(feed_dict={
                vgg.imgs: tn_batch, 
                vgg.a11: attnmats[0],
                vgg.a12: attnmats[1],
                vgg.a21: attnmats[2],
            ...
            if rec_activity: 
                resp_list = sess.run([
                    vgg.smean1_1, vgg.smean1_2, vgg.smean2_1, vgg.smean2_2,
                    vgg.smean3_1, vgg.smean3_2, vgg.smean3_3,
                    vgg.smean4_1, vgg.smean4_2, vgg.smean4_3,
                    vgg.smean5_1, vgg.smean5_2, vgg.smean5_3
                ], feed_dict={
                    vgg.imgs: tp_batch, 
                    vgg.a11: attnmats[0],
                    vgg.a12: attnmats[1],
                    ...
                    vgg.a52: attnmats[11],
                    vgg.a53: attnmats[12]
                })  # layer, cat, batchim, featmap

                # Assign responses for true positives
                tp_resps1[0, :, :, ai] = resp_list[0]
                tp_resps1[1, :, :, ai] = resp_list[1]
                tp_resps2[0, :, :, ai] = resp_list[2]
                tp_resps2[1, :, :, ai] = resp_list[3]
                tp_resps3[0, :, :, ai] = resp_list[4]
                tp_resps3[1, :, :, ai] = resp_list[5]
                tp_resps3[2, :, :, ai] = resp_list[6]
                tp_resps4[0, :, :, ai] = resp_list[7]
                tp_resps4[1, :, :, ai] = resp_list[8]
                tp_resps4[2, :, :, ai] = resp_list[9]
                tp_resps5[0, :, :, ai] = resp_list[10]
                tp_resps5[1, :, :, ai] = resp_list[11]
                tp_resps5[2, :, :, ai] = resp_list[12]

                # Assign responses for true negatives
                resp_list = sess.run([
                    vgg.smean1_1, vgg.smean1_2, vgg.smean2_1, vgg.smean2_2,
                    vgg.smean3_1, vgg.smean3_2, vgg.smean3_3,
                    vgg.smean4_1, vgg.smean4_2, vgg.smean4_3,
                    ...
                    vgg.a52: attnmats[11],
                    vgg.a53: attnmats[12]
                })  # layer, cat, im, featmap

                tn_resps1[0, :, :, ai] = resp_list[0]
                tn_resps1[1, :, :, ai] = resp_list[1]
                tn_resps2[0, :, :, ai] = resp_list[2]
                tn_resps2[1, :, :, ai] = resp_list[3]
                tn_resps3[0, :, :, ai] = resp_list[4]
                tn_resps3[1, :, :, ai] = resp_list[5]
                tn_resps3[2, :, :, ai] = resp_list[6]
                tn_resps4[0, :, :, ai] = resp_list[7]
                tn_resps4[1, :, :, ai] = resp_list[8]
                tn_resps4[2, :, :, ai] = resp_list[9]
                tn_resps5[0, :, :, ai] = resp_list[10]
                tn_resps5[1, :, :, ai] = resp_list[11]
